...
** *Periode* : "Heures Pleines" or "Heures Creuses" stands for high/low cost hours
** *Total* : Teleinfo - i.e. Electricity provider serial info of current consumption
** *Non-Heating* : Clamp meter on non-heating part of the house
** *Consumers* : Clamp meters on various elements
** *Outlets* : Wall plugs
** *Cost HCHP* : Computed costs, taking Low/High hours into account
** *Cost if Base* : Computed costs, if using a contract without Low/High hours
** *Index* : Rolling indexes, if need to provide
